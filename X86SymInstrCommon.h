...
enum X86RegisterClass
{
  ...
  // Don't try to combine these using X86_SYMINSTR_PREFIX, there be dragons here
		...
		#define X86_SYMINSTR_NAME_2SIZE_OP(name, destSize, srcSize, op) \
			X86Sym_##name##_##destSize##_##srcSize##_##op
		...
		#define X86_SYMINSTR_CLASS_2SIZE_OP(name, destSize, srcSize, op) \
			X86Sym_##name##_##destSize##_##srcSize##_##op##_Instr
	#else
		...
		#define X86_SYMINSTR_NAME_2SIZE_OP(name, destSize, srcSize, op) \
			X64Sym_##name##_##destSize##_##srcSize##_##op
		...
		#define X86_SYMINSTR_CLASS_2SIZE_OP(name, destSize, srcSize, op) \
			X64Sym_##name##_##destSize##_##srcSize##_##op##_Instr
	#endif

	#define X86_MEM_OP_PARAM \
		uint32_t base, uint32_t index, uint32_t scale, uint32_t stackVar, int64_t offset
	...
	#define X86_SYM_MEM_INDEX(base, index, scale, stackVar, offset) \
		base, index, scale, stackVar, offset
...
class X86_SYMINSTR_NAME(Instr) : public SymInstr
{
 protected:
	...
	#define X86_DECLARE_MOV_SIZE(name, size) \
		...
		{ \
		 public: \
			...
			virtual bool EmitInstruction(SymInstrFunction* func, OutputBlock* out); \
			virtual void Print(SymInstrFunction* func); \
			virtual bool UpdateInstruction( \
			    SymInstrFunction* func, const Settings& settings, std::vector<SymInstr*>& replacement); \
		}; \
	...
	#define X86_DECLARE_MOVEXT_SIZE(name, destSize, srcSize) \
		class X86_SYMINSTR_CLASS_2SIZE_OP(name, destSize, srcSize, RR) : \
		    public X86_SYMINSTR_NAME(Instr) \
		{ \
		 public: \
		...
		class X86_SYMINSTR_CLASS_2SIZE_OP(name, destSize, srcSize, RM) : \
		    public X86_SYMINSTR_NAME(Instr) \
		{ \
		 public: \
	...
	#define X86_DECLARE_SHIFT_DOUBLE_SIZE(name, size) \
		...
		SymInstr* X86_SYMINSTR_NAME_SIZE_OP(name, size, MRR)( \
...
{
 public:
	X86_SYMINSTR_CLASS(MovCodePtrBaseRelative)
...
{
 public:
	X86_SYMINSTR_CLASS(CallDirect)
	(Function* func, ILBlock* block, uint32_t retVal, uint32_t retValHigh, uint32_t key,
...
{
 public:
	X86_SYMINSTR_CLASS(CallIndirectReg)
	(uint32_t func, uint32_t retVal, uint32_t retValHigh, uint32_t key, uint32_t scratch,
...
{
 public:
	X86_SYMINSTR_CLASS(CallIndirectMem)
	(X86_MEM_OP_PARAM, uint32_t retVal, uint32_t retValHigh, uint32_t key, uint32_t scratch,
	    const std::vector<uint32_t>& reads);
	virtual bool EmitInstruction(SymInstrFunction* func, OutputBlock* out);
	virtual void Print(SymInstrFunction* func);
};
...
class X86_SYMINSTR_CLASS(Syscall) : public X86_SYMINSTR_NAME(Instr)
{
 ...
 public:
	X86_SYMINSTR_CLASS(Syscall)
	(uint32_t eax, uint32_t edx, uint32_t ecx, const std::vector<uint32_t> readRegs,
...
class X86_SYMINSTR_CLASS(SyscallInt80) : public X86_SYMINSTR_NAME(Instr)
{
 ...
 public:
	X86_SYMINSTR_CLASS(SyscallInt80)
	(uint32_t eax, uint32_t edx, uint32_t ecx, const std::vector<uint32_t> readRegs,
	    const std::vector<uint32_t> spilledRegs);
	virtual bool EmitInstruction(SymInstrFunction* func, OutputBlock* out);
	virtual void Print(SymInstrFunction* func);
...
class X86_SYMINSTR_NAME(Function) : public SymInstrFunction
{
 public:
	...
	virtual bool GenerateSpillLoad(uint32_t reg, uint32_t var, int64_t offset, ILParameterType type,
	    std::vector<SymInstr*>& code);
	virtual bool GenerateSpillStore(uint32_t reg, uint32_t var, int64_t offset, ILParameterType type,
	    std::vector<SymInstr*>& code);
...
SymInstr* X86_SYMINSTR_NAME(CallDirect)(Function* func, ILBlock* block, uint32_t retVal,
    uint32_t retValHigh, uint32_t key, uint32_t scratch, const std::vector<uint32_t>& reads);
SymInstr* X86_SYMINSTR_NAME(CallIndirectReg)(uint32_t func, uint32_t retVal, uint32_t retValHigh,
    uint32_t key, uint32_t scratch, const std::vector<uint32_t>& reads);
...
SymInstr* X86_SYMINSTR_NAME(Syscall)(uint32_t eax, uint32_t edx, uint32_t ecx,
    const std::vector<uint32_t>& readRegs, const std::vector<uint32_t>& spilledRegs);
